  ...
  const renderCertificate = useCallback(async () => {
    ...
    try {
      ...
      if (templateConfig) {
        ...
        const drawTextField = (text: string, config: any) => {
          if (!text) return;
        ...
        drawTextField(data.name, templateConfig.name);
        drawTextField(data.content, templateConfig.content);
        drawTextField(data.guidanceUnit ? `指导单位：${data.guidanceUnit}` : '', templateConfig.guidanceUnit);
        drawTextField(data.certNumber ? `证书编号：${data.certNumber}` : '', templateConfig.certNumber);
        drawTextField(data.authTime ? `授权时间：${dateToChinese(data.authTime)}` : '', templateConfig.authTime);
        drawTextField(data.authUnit, templateConfig.authUnit);
